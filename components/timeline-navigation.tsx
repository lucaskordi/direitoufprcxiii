...
const timelineData: YearData[] = [
  {
    ...
    events: [
      ...
      { id: 7, title: "Med in Eden", date: "07.11", isCompleted: true },
  ...
  {
    year: "2025",
    events: [{ id: 13, title: "Banho de Lama", date: "16.01" }],
  },
  {
    year: "2026",
    events: [
  ...
  {
    year: "2029",
    events: [{ id: 19, title: "Festa de 365 e/ou 100 Dias", date: "Ano todo" }],
  },
  {
    year: "2030",
    events: [
...
export default function TimelineNavigation({
  ...
  useEffect(() => {
    if (isOpen && scrollRef.current) {
      ...
      setTimeout(() => {
        const activeEvent = scrollRef.current?.querySelector(
  ...
  return (
    <div className="fixed top-8 left-8 z-30">
      ...
      {(isOpen || isClosing) && (
        <div
          className={`absolute top-12 left-0 glass-dark rounded-2xl p-6 w-80 max-h-96 overflow-y-auto timeline-scroll scroll-smooth relative ${
          ...
          {showScrollIndicator && (
            <div className="absolute bottom-2 right-2 z-10">
              <div className="glass-orange rounded-full p-2 animate-bounce">
                <svg
                  className="w-4 h-4 text-formo-cream"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            {timelineData.map((yearData) => (
              <div key={yearData.year}>
                <button
                  onClick={() =>
                    setSelectedYear(
                      selectedYear === yearData.year ? null : yearData.year
                    )
                  }
